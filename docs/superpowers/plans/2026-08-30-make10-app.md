...
```bash
C:/flutter/bin/flutter.bat test && flutter analyze
```
...
- [ ] `flutter test` が全件 PASS
- [ ] `flutter analyze` が `No issues found!`
- [ ] `dart run tool/generate_puzzles.dart` を再実行しても `assets/puzzles.json` に差分が出ない
- [ ] Task 16 Step 9 の手動確認 11 項目がすべて通る
