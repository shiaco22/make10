...
/// 正規形キー専用の記号。表示用の opSymbol（`×` `÷`）とは分ける。
///
/// キーが表示フォーマットに依存すると、見た目を変えたときに重複排除の
/// 挙動が黙って変わる。テストが期待する `(((5/5)+1)*5)` もこの ASCII 形。
...
void main() {
  ...
  test('isDeadEnd detects a mid-game dead end', () {
    // 3,4,7,9 は解けるが、3*7=21 に進むと 4,9,21 になり 10 は作れない。
    // 3*4=12 を選ぶと 12,7,9 になり、これは 12+7-9=10 など 5 通りで解ける。
...
- **`cast<T>()` は遅延評価。** `List` を型付きリストに変換する箇所で `cast<T>()` を使うと、要素の型チェックが実際の走査時まで起きない。`try`/`catch` で囲んだ読み込み処理の中で使っても例外はそこで飛ばず、ずっと後の利用箇所で `TypeError` として噴き出し、フォールバックが働かない。JSON の読み込みでは **`List<T>.from(...)` を使って即時変換する**こと。

- **1 つの出題が統計に寄与するのは 1 回だけ、という不変条件を明示的に持たせる。** Task 11 の初版は `undo` / `resetBoard` を `cleared` 局面で止めていなかったため、クリア → undo → 再クリアを繰り返すだけで `solved` と `totalTimeMs` を無制限に水増しできた。局面ガードだけでは clear → undo → skip の経路が残る。記録済みフラグを `_deal` でリセットする形にすること。
- **一時状態は「選択」だけではない。** `undo` / `resetBoard` が選択のみを消してヒントを残していたため、undo 直後に「詰み」表示が残り、解ける盤面を解けないと案内していた。ヒントの指すインデックスも古い盤面のものになる。選択・拒否理由・ヒント・詰み表示をまとめて捨てる私的ヘルパーを用意し、`showAnswer` からも呼ぶこと。
- **`ChangeNotifier` を非同期処理と組み合わせるときは破棄ガードを持たせる。** Task 12 の初版は保存完了後に `notifyListeners()` を呼んでいたが、リザルト画面はルートの pop でセッションを dispose するため、時間切れ直後に戻ると破棄済みの通知器を叩いて落ちた。`dispose()` の冪等化と、`tick` / リスナー / 非同期コールバックすべてでのガードが要る。書き込み失敗時にも「保存中」フラグを解除すること。
- **リストで描くウィジェットには `Key` を付ける。** 盤面はマージのたびに 2 枚減って 1 枚増えるため、生き残ったカードが前の位置にずれる。`Key` がないと Flutter は位置で対応づけ、ずれ込んだ先にいた別カードのアニメーション状態を引き継いで毎回ちらつく。カード id は undo をまたぐと使い回されるが、間に必ず「その id が木に存在しないフレーム」が挟まるのでキーの連続性は切れる。**フレームをまたいで保持する状態**（進行中のアニメーションフラグ、id で引くコントローラ）に id を使うのは依然として危険。
- **`Semantics` の `label` は子の意味を打ち消さない。** `excludeSemantics: true` を付けないと子の `Text` のラベルが合成され、読み上げが「カード 9 / 9」と二重になる。ただし `excludeSemantics` は子の**動作**も消すので、`InkWell` が提供していたタップを外側の `Semantics(onTap: ...)` で補うこと。
- **状態が重なったときの配色を確かめる。** 背景を if/else で決めて枠線を別条件で決めると、選択中かつヒント強調中のように両方立つ組み合わせで枠線と背景が同系色になり見えなくなる。ヒントが示した 2 枚の片方をタップした瞬間がまさにこれで、残り 1 枚へ導くべき強調が消えていた。
- **一時メッセージを 1 つのスロットに優先順位で出すなら、各アクションが自分と競合するメッセージを消すこと。** `requestHint()` だけが `_lastRejection` を消し忘れていたため、合成を拒否された直後にヒントを押すと、拒否メッセージが詰み通知に勝って「ヒントが効かない」ように見えた。ヒントが実際に見つかったケースでも同じで、カードは強調されるのに通知欄は古い拒否理由のままだった。優先順位の入れ替えでは直らない（ヒント成功時は詰みフラグが偽なので同じ分岐に落ちる）。
- **固定幅のボタンを並べた `Row` は狭い端末で溢れる。** 演算子バーは 4×76=304px を要求するが、320pt 幅の端末では本文パディングを引いて 288px しかなく、16px はみ出していた。`FittedBox(fit: BoxFit.scaleDown)` で包むと、収まらないときだけ縮み、広い画面では何も変わらない。
